...
async def async_get_dhcp(hass: HomeAssistant) -> list[DHCPMatcher]:
    ...
    for integration in integrations.values():
        ...
        for entry in integration.dhcp:
            dhcp.append(
                cast(DHCPMatcher, {"domain": integration.domain, **entry}))
...
async def async_get_homekit(
    ...
    homekit: dict[str, HomeKitDiscoveredIntegration] = {
        model: HomeKitDiscoveredIntegration(
            cast(str, details["domain"]), cast(
                bool, details["always_discover"])
        )
...
class Integration:
    ...
    def resolve_from_root(
        ...
        for base in root_module.__path__:
            ...
            try:
                manifest = cast(Manifest, json_loads(
...
class Components:
    ...
    def __getattr__(self, comp_name: str) -> ModuleWrapper:
        ...
        if isinstance(integration, Integration):
            ...
            component = _load_file(self._hass, comp_name,
                                   _lookup_path(self._hass))
