...
def generate(template: str, info: Info) -> None:
    ...
    _generate(TEMPLATE_DIR / template / "integration",
              info.integration_dir, info)
...
def _custom_tasks(template, info: Info) -> None:
    ...
    if template == "integration":
        # changes = {"codeowners": [info.codeowner], "iot_class": info.iot_class} # the default
        changes = {"iot_class": info.iot_class}
        ...
        info.update_manifest(config_flow=True, dependencies=[
